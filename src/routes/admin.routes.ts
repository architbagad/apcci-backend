...
adminRouter.put("/user/:id", async (c) => {
  ...
  if (!updatedUser) {
    return c.json({ success: false, message: "User not found or update failed" }, 404);
  }

  return c.json({ success: true, message: "User updated successfully", user: updatedUser });
});
...
adminRouter.post("/allot-bin", async (c) => {
  ...
  return c.json({ success: true, message: "Bin allotted to user successfully" });
});
...
adminRouter.delete("/remove-bin", async (c) => {
  ...
  if (!success) {
    return c.json({ success: false, message: "Failed to remove bin from user" }, 500);
  }

  return c.json({ success: true, message: "Bin removed from user successfully" });
});
...
adminRouter.post("/bin", async (c) => {
  ...
  if (!location || !tag) {
    return c.json({ success: false, message: "Location and Tag are required" }, 400);
  }
...
adminRouter.put("/bin/:id", async (c) => {
  ...
  if (!updatedBin) {
    return c.json({ success: false, message: "Bin not found or update failed" }, 404);
  }

  return c.json({ success: true, message: "Bin updated successfully", bin: updatedBin });
});
...
adminRouter.delete("/allotment/:id", async (c) => {
  ...
  if (!success) {
    return c.json({ success: false, message: "Failed to delete allotment" }, 500);
  }
